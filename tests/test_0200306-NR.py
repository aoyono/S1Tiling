...
from .mock_otb import (
    OTBApplicationsMockContext,
    isfile,
    isdir,
    list_dirs,
    glob,
...
def process(
    ...
    logging.info(
        "$S1TILING_TEST_DATA_INPUT  -> %s", os.environ["S1TILING_TEST_DATA_INPUT"]
    )
    logging.info(
        "$S1TILING_TEST_DATA_OUTPUT -> %s", os.environ["S1TILING_TEST_DATA_OUTPUT"]
    )
    ...
    logging.info(
        "$S1TILING_TEST_DOWNLOAD    -> %s", os.environ["S1TILING_TEST_DOWNLOAD"]
    )
...
def test_33NWB_202001_NR(
    baselinedir, outputdir, tmpdir, srtmdir, ram, download, watch_ram
):
    crt_dir = pathlib.Path(__file__).parent.absolute()
    ...
    assert os.path.isdir(baselinedir), (
        ...
        + "?> mc cp --recursive minio-otb/s1-tiling/baseline '%s'"
    ) % (
        baselinedir,
        baselinedir.absolute(),
    )
    ...
    for im in images:
        ...
        assert (
            otb_compare(expected, produced) == 0
...
def test_33NWB_202001_NR_masks_only(
    baselinedir, outputdir, tmpdir, srtmdir, ram, download, watch_ram
):
    crt_dir = pathlib.Path(__file__).parent.absolute()
    logging.info("Baseline expected in '%s'", baselinedir)
    ...
    dirs_to_clean = [
        tmpdir / "S1",
        tmpdir / "S2",
    ]  # do not clear outputdir in that case
...
class FileDB:
    ...
    def raster_vv(self, idx):
        ...
        return (
            ...
            [
                (14.9998201759, 1.8098185887),
                (15.9870050338, 1.8095484335),
                (15.9866155411, 0.8163071941),
                (14.9998202469, 0.8164290331000001),
            ],
        )
    ...
    def tmp_orthofile(self, idx):
        return (
            f'{self.__tmp_dir}/S2/{self.__tile}/{self.FILES[idx]["tmp_orthofile"]}.tif'
            + self.extended_geom_compress
        )
    ...
    def concatfile(self, idx):
        if idx is None:
            ...
            return (
                f'{self.__output_dir}/{self.__tile}/{self.FILES[idx]["orthofile"]}.tif'
            )

    def tmp_concatfile(self, idx):
        if idx is None:
            return (
                f"{self.__tmp_dir}/S2/{self.__tile}/s1a_33NWB_vv_DES_007_20200108txxxxxx.tmp.tif"
                + self.extended_compress
            )
        else:
            return (
                f'{self.__tmp_dir}/S2/{self.__tile}/{self.FILES[idx]["orthofile"]}.tmp.tif'
                + self.extended_compress
            )
...
def _declare_know_files(mocker, known_files, known_dirs, patterns, file_db):
    ...
    mocker.patch(
        "s1tiling.libs.S1FileManager.list_dirs",
        lambda dir, pat: list_dirs(dir, pat, known_dirs, file_db.inputdir),
    )
    ...
    mocker.patch(
        "s1tiling.libs.otbwrappers.OrthoRectify.add_ortho_metadata",
        lambda slf, mt: True,
    )
    mocker.patch(
        "s1tiling.libs.otbpipeline.commit_otb_application", lambda tmp, out: True
    )


def test_33NWB_202001_NR_mocked(
    baselinedir, outputdir, tmpdir, srtmdir, ram, download, watch_ram, mocker
):
    crt_dir = pathlib.Path(__file__).parent.absolute()
    logging.info("Baseline expected in '%s'", baselinedir)
    ...
    application_mocker = OTBApplicationsMockContext(
        configuration, mocker, file_db.tmp_to_out_map
    )
    ...
    s1tiling.S1Processor.s1_process(
        config_opt=configuration,
        searched_items_per_page=0,
        dryrun=False,
        debug_otb=True,
        watch_ram=False,
        debug_tasks=False,
