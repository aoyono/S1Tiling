...
from s1tiling.libs.otbpipeline import (
...
def raster_vv(idx):
    ...
    return (
        S1DateAcquisition(
            f"{INPUT}/{s1dir}/{s1dir}.SAFE/manifest.safe", [input_file(idx)]
        ),
        [
...
def given_pipeline_concat(pipelines, builds):
    if builds == "builds":
        ...
        pipelines.register_pipeline(
            [BuildBorderMask, SmoothBorderMask], "GenerateMask", product_required=True
        )
...
def when_analyse_dependencies(
    pipelines, raster_list, dependencies, mocker, known_files
):
...
@then(
    parsers.parse(
        "it depends on 2 ortho files (and two S1 inputs), and {a} mask on a concatenated product"
    )
)
...
@then(
    parsers.parse(
        "it depends on one ortho file (and one S1 input), and {a} mask on a concatenated product"
    )
)
...
@then(
    parsers.parse(
        "it depends on second ortho file (and second S1 input), and {a} mask on a concatenated product"
    )
)
...
@then(
    parsers.parse(
        "a concatenation task is registered and produces txxxxxxx S2 file and {a} mask"
    )
)
def then_concatenate_2_files_(tasks, dependencies, a):
    ...
    if a != "no":
        expectations[maskfile(None)] = {
            "pipeline": "GenerateMask",
            "input_step": FirstStep,
        }
    required, previous, task2outfile_map = dependencies
...
@then(
    parsers.parse(
        "a concatenation task is registered and produces t-chrono S2 file, and {a} mask"
    )
)
def then_concatenate_1_files(tasks, dependencies, known_file_numbers, a):
    ...
    if a != "no":
        expectations[maskfile(known_file)] = {
            "pipeline": "GenerateMask",
            "input_step": FirstStep,
        }
