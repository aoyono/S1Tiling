...
def comparable_metadata(image):
    ...
    if "TIFFTAG_SOFTWARE" in md:
        ...
        logging.info(
            'TIFFTAG_SOFTWARE changed from "%s" to "%s" [in "%s"]'
            % (md["TIFFTAG_SOFTWARE"], ts, image)
        )
...
def metadata_compare(baseline, result):
    ...
    arg = (
        'bash -c "diff -I PROCESSED_DATETIME -I TIFFTAG_DATETIME -I "Files:" <(gdalinfo %s) <(gdalinfo %s)"'
        % (baseline, result)
    )
    print(arg)
