...
def extract_tiles_to_process(cfg, s1_file_manager):
    ...
    if all_requested:
        if cfg.download and "ALL" in cfg.roi_by_tiles:
            logger.critical(
                "Can not request to download 'ROI_by_tiles : ALL' if 'Tiles : ALL'."
            ...
            logger.info(
                "All tiles for which more than %s%% of the surface is covered by products will be produced: %s",
                100 * cfg.TileToProductOverlapRatio,
                tiles_to_process,
            )
...
def check_tiles_to_process(tiles_to_process, s1_file_manager):
    ...
    for tile in tiles_to_process:
        ...
        if current_coverage < 1.0:
            logger.warning(
                "Tile %s has insuficient SRTM coverage (%s%%)",
                tile,
                100 * current_coverage,
            )
...
def setup_worker_logs(config, dask_worker):
    ...
    for hdlr in old_handlers:
        ...
        r_logger.addHandler(
            hdlr
        )  # <-- this way we send s1tiling messages to dask channel
...
def process_one_tile(
    ...
    try:
        with Utils.ExecutionTimer("Downloading images related to " + tile_name, True):
            s1_file_manager.download_images(
                tiles=tile_name,
    ...
    dsk, required_products = pipelines.generate_tasks(
        tile_name,
        intersect_raster_list,
        debug_otb=debug_otb,
        dryrun=dryrun,
        do_watch_ram=do_watch_ram,
    )
    logger.debug(
        "Summary of tasks related to S1 -> S2 transformations of %s", tile_name
    )
    results = []
    if debug_otb:
        tasks = list(
            Utils.tsort(
                dsk,
                dsk.keys(),
                lambda dask_task_data: []
                if isinstance(dask_task_data, FirstStep)
                else dask_task_data[2],
            )
        )
        ...
        logger.info(
            "Executing tasks one after the other for %s (debugging OTB)", tile_name
        )
        for product in reversed(tasks):
            how = dsk[product]
        ...
        if debug_tasks:
            SimpleComputationGraph().simple_graph(
                dsk, filename="tasks-%s-%s.svg" % (tile_idx + 1, tile_name)
            )
        ...
        for run_attemp in range(1, nb_tries + 1):
            try:
                ...
                logger.exception(
                    "Worker %s has been killed when processing %s on %s tile: (%s). Workers will be restarted: %s/%s",
                    e.last_worker.name,
                    e.task,
                    tile_name,
                    e,
...
def s1_process(
    ...
    with S1FileManager(config) as s1_file_manager:
        ...
        logger.info(
            "%s images to process on %s tiles",
            s1_file_manager.nb_images,
        ...
        if not os.path.exists(config.GeoidFile):
            logger.critical(
                "Geoid file does not exists (%s), exiting ...", config.GeoidFile
            )
        ...
        if cache_before_ortho:
            pipelines.register_pipeline(
                [AnalyseBorders, Calibrate, CutBorders],
                "PrepareForOrtho",
                product_required=False,
            )
            pipelines.register_pipeline(
                [OrthoRectify], "OrthoRectify", product_required=False
            )
        else:
            pipelines.register_pipeline(
        ...
        if config.mask_cond:
            pipelines.register_pipeline(
                [BuildBorderMask, SmoothBorderMask],
                "GenerateMask",
        ...
        try:
            if not debug_otb:
                ...
                cluster = LocalCluster(
                    ...
                    processes=True,
                    n_workers=config.nb_procs,
                    silence_logs=False,
                )
                client = Client(cluster)
                client.register_worker_callbacks(
                    lambda dask_worker: setup_worker_logs(
                        config.log_config, dask_worker
                    )
                )
            else:
                client = None
            ...
            if nb_error_detected > 0:
                logger.warning(
                    "Execution report: %s errors detected", nb_error_detected
                )
            else:
