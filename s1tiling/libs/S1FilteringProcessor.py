...
class S1FilteringProcessor:
    ...
    def process(self, tile):
        ...
        for y in year_outcore_list:
            for file_it in glob.glob(
                os.path.join(directory, "s1?_?????_??_DES_???_" + y + "????t??????.tif")
            ):
                filelist_s1des_updateoutcore.append(file_it)

            for file_it in glob.glob(
                os.path.join(directory, "s1?_?????_??_ASC_???_" + y + "????t??????.tif")
            ):
        ...
        if self.Cg_Cfg.Reset_outcore:
            ...
            try:
                processed_files = pickle.load(
                    open(os.path.join(directory, "outcore" + year_filter + ".txt"))
                )
        ...
        if filelist_s1des_updateoutcore:
            command = (
                "export ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS={};".format(OTBThreads)
                + "otbcli_MultitempFilteringOutcore -progress false -inl "
                + filelist_s1des_updateoutcore_str
                + " -oc "
            ...
            pids.append(
                [
                    Popen(
                        command,
                        stdout=self.Cg_Cfg.stdoutfile,
                        stderr=self.Cg_Cfg.stderrfile,
                        shell=True,
                    ),
                    command,
                ]
        ...
        if filelist_s1asc_updateoutcore:
            command = (
                "export ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS={};".format(OTBThreads)
                + "otbcli_MultitempFilteringOutcore -progress false -inl "
                + filelist_s1asc_updateoutcore_str
                + " -oc "
                + os.path.join(directory, "outcore" + year_outcore_str + "_S1ASC.tif")
                + " -wr "
                + str(self.Cg_Cfg.Window_radius)
                + " -ram {}".format(str(ram_per_process))
            )
            pids.append(
                [
        ...
        if filelist_s1des:
            command = (
                ...
                + " -enl "
                + os.path.join(
                    directory, "filtered", "enl_" + year_outcore_str + "_S1DES.tif"
                )
                + " -ram {}".format(str(ram_per_process))
            )
            pids.append(
        ...
        if filelist_s1asc:
            command = (
                ...
                + " -enl "
                + os.path.join(
                    directory, "filtered", "enl_" + year_outcore_str + "_S1ASC.tif"
                )
                + " -ram {}".format(str(ram_per_process))
            )
            pids.append(
        ...
        for f in os.listdir(filtering_directory):
            ...
            if (
                ...
                and f.startswith("s1")
                and f.endswith("filtered.tif")
            ):
                dst = gdal.Open(fullpath, gdal.GA_Update)
                dst.SetMetadataItem("FILTERED", "true")
                dst.SetMetadataItem(
                    "FILTERING_WINDOW_RADIUS", str(self.Cg_Cfg.Window_radius)
                )
                dst.SetMetadataItem(
                    "FILTERING_PROCESSINGDATE", str(datetime.datetime.now())
                )
