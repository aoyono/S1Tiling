...
def does_final_product_need_to_be_generated_for(
    product, tile_name, polarizations, s2images
):
    """
    ...
    for pol in polarizations:
        ...
        pat_filtered = "%s_%s_%s_*_%st??????_filtered.tif" % (
            sat.lower(),
            tile_name,
            "vh",
            start,
...
def filter_images_or_ortho(kind, all_images):
    ...
    if not images:
        images = [
            f.replace("_OrthoReady.tiff", ".tiff")
...
def _download_and_extract_one_product(dag, raw_directory, product):
    ...
    ok_msg = "Successful download (and extraction) of %s" % (
        product,
...
class S1FileManager:
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.download:
            logger.debug(
                "Using %s EODAG configuration file",
                self.cfg.eodagConfig or "user default",
            )
    ...
    def tmpsrtmdir(self, srtm_tiles_id, srtm_suffix=".hgt"):
        ...
        if not self.__tmpsrtmdir:
            ...
            logger.debug(
                "Create temporary SRTM diretory (%s) for needed tiles %s",
                self.__tmpsrtmdir.name,
                srtm_tiles_id,
            )
            ...
            for srtm_tile in srtm_tiles_id:
                ...
                srtm_tile_filelink = Path(
                    self.__tmpsrtmdir.name, srtm_tile + srtm_suffix
                )
                if self.__caching_option == "symlink":
                    logger.debug(
                        "- ln -s %s <-- %s", srtm_tile_filepath, srtm_tile_filelink
                    )
                    srtm_tile_filelink.symlink_to(srtm_tile_filepath)
                else:
                    logger.debug(
                        "- cp %s <-- %s", srtm_tile_filepath, srtm_tile_filelink
                    )
    ...
    def keep_X_latest_S1_files(self, threshold):
        ...
        safefile_list = sorted(
            glob.glob(os.path.join(self.cfg.raw_directory, "*")), key=os.path.getctime
        )
    ...
    def _download(
        ...
        while True:
            ...
            dag_polarization_param = (
                "VV VH" if polarization in ["VV VH", "VV", "VH"] else "HH HV"
            )
            ...
            logger.info(
                "%s remote S1 products returned in page %s: %s",
                len(page_products),
                page,
                page_products,
            )
        ...
        logger.debug(
            "%s remote S1 product(s) left after discarding smallest redundant ones: %s",
            len(products),
            products,
        )
        ...
        products = [p for p in products if not p.as_dict()["id"] in self.product_list]
        logger.debug(
            "%s remote S1 product(s) are not yet in the cache: %s",
            len(products),
            products,
        )
        if not products:  # no need to continue
        ...
        s2images = glob.glob1(tile_out_dir, s2images_pat) + glob.glob1(
            os.path.join(tile_out_dir, "filtered"), s2images_pat
        )
        products = [
            p
            for p in products
        ...
        logger.info(
            "%s remote S1 product(s) will be downloaded: %s", len(products), products
        )
        if not products:  # no need to continue
        ...
        paths = _parallel_download_and_extraction_of_products(
            ...
            self.cfg.raw_directory,
            products,
            self.cfg.nb_download_processes,
            tile_name,
        )
    ...
    def _update_s1_img_list(self):
        ...
        self.product_list = []
        content = list_dirs(
            self.cfg.raw_directory, "S1*_IW_GRD*"
        )  # get rid of `.download` on the-fly
        ...
        for current_content in content:
            ...
            safe_dir = os.path.join(
                current_content.path, os.path.basename(current_content.path) + ".SAFE"
            )
            ...
            vv_images = (
                filter_images_or_ortho("vv", all_tiffs)
                if self.cfg.polarisation in ["VV", "VV VH"]
                else []
            )
            vh_images = (
                filter_images_or_ortho("vh", all_tiffs)
                if self.cfg.polarisation in ["VH", "VV VH"]
                else []
            )
            hv_images = (
                filter_images_or_ortho("hv", all_tiffs)
                if self.cfg.polarisation in ["HV", "HH HV"]
                else []
            )
            hh_images = (
                filter_images_or_ortho("hh", all_tiffs)
                if self.cfg.polarisation in ["HH", "HH HV"]
                else []
            )
    ...
    def get_tiles_covered_by_products(self):
        ...
        for image in self.get_raster_list():
            ...
            for current_tile in layer:
                ...
                if (
                    intersection.GetArea() / tile_footprint.GetArea()
    ...
    def is_product_in_time_range(self, product):
        ...
        logger.debug(
            "  %s %s /// %s == %s <= %s <= %s",
            "KEEP" if is_in_range else "DISCARD",
            path,
            is_in_range,
            self.first_date,
            start,
    ...
    def get_s1_intersect_by_tile(self, tile_name_field):
        ...
        for image in self.get_raster_list():
            ...
            if len(image.get_images_list()) == 0:
                ...
                logger.critical(
                    "Please remove the raw data for %s SAFE file", image.get_manifest()
                )
            ...
            intersection = poly.Intersection(tile_footprint)
            logger.debug(
                "   -> Test intersection: requested: %s  VS tile: %s --> %s",
                poly,
                tile_footprint,
                intersection,
            )
    ...
    def record_processed_filenames(self):
        ...
        with open(
            os.path.join(self.cfg.output_preprocess, "processed_filenames.txt"), "a"
        ) as in_file:
    ...
    def get_processed_filenames(self):
        ...
        try:
            with open(
                os.path.join(self.cfg.output_preprocess, "processed_filenames.txt"), "r"
            ) as in_file:
